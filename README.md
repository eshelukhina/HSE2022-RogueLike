...
- **EventHandler** - интерфейс, отвечающий за исполнение ввода пользователя
  ...
  - **SystemHandler** - отвечает за работу с главным меню (`Menu`)
  - **InventoryHandler** - будет отвечаеть за работу с инвентарем – позволяет надевать/снимать/выбрасывать экиперовку и оружие
...
- **Cell** - хранит расположение клетки на сетке карты, а также ее содержимое (`CellType`)
- **Enemy** - базовый класс, от которого будут наследоваться мобы
